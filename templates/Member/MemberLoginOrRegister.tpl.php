...
<form id="order_<?= $action ?>" method="post" action="<?= $_SERVER['REQUEST_URI'] ?>">
	...
	<label for="login">Login :</label><input id="login" name="login" class="form-control" type="email" required="required" value="<?= $data['login'] ?>" /> <br>
	<label for="password">Mot de passe :</label><input id="password" name="password" type="password" required="required" value="<?= $data['password'] ?>" /> <br>
	</fieldset>
